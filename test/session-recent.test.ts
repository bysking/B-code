...
function round(question: string, answer: string, tools: string[] = []): MessageParam[] {
  ...
  if (tools.length) msgs.push({ role: "user", content: tools.map((t) => ({ type: "tool_result", tool_use_id: t, content: "ok" })) });
  return msgs;
}
...
test("recentTurns：空/无 user 文本 → []", () => {
  ...
  assert.deepEqual(recentTurns([{ role: "user", content: [{ type: "tool_result", tool_use_id: "1", content: "x" }] }], 5), []);
});

test("renderRecentTurns：紧凑文本含标注（轮数 = user 消息数）", () => {
  const out = renderRecentTurns([{ role: "user", text: "你好", tools: [] }, { role: "assistant", text: "hello", tools: ["read_file"] }]);
